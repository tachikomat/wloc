const FRAME_PREFIX_LENGTH = 8
const FRAME_LENGTH_LENGTH = 2
const FRAME_HEADER_LENGTH = FRAME_PREFIX_LENGTH + FRAME_LENGTH_LENGTH
const FRAME_SCAN_OFFSETS = [0, 2, 4, 6, 8, 10, 12, 14, 16]
const MAX_FRAME_SCAN_OFFSET = 96
const MAX_RAW_PROTOBUF_SCAN_OFFSET = 256
 ...
 * @param {{ longitude: number; latitude: number; accuracy: number; altitude: number }} target
 */
export async function patchWithStats(input, target) {
...
export async function extractLocations(body) {
  const input = toBytes(body)
  const bytes = isGzip(input) ? await ungzip(input) : input
  const errors = []

  for (const offset of candidateFrameOffsets(bytes)) {
    try {
      const payload = readFramePayload(bytes, offset)
      const locations = collectAppleWLocLocations(payload)
  ...
  for (
    let offset = 0;
    offset <= Math.min(MAX_RAW_PROTOBUF_SCAN_OFFSET, bytes.length);
    offset++
  ) {
    try {
      const locations = collectAppleWLocLocations(bytes.slice(offset))
      if (locations.length > 0) return locations
    } catch (error) {
  ...
  throw new Error(`no wloc locations found; ${errors.join(" | ")}`)
}
...
function patchFrame(bytes, offset, target, stats) {
  ...
  if (patchedPayload.length > 0xffff) {
  ...
  return concatBytes([
    ...
    Uint8Array.of(
      (patchedPayload.length >> 8) & 0xff,
      patchedPayload.length & 0xff,
    ),
    patchedPayload,
...
function readFramePayload(bytes, offset) {
  ...
  const length =
    (bytes[offset + FRAME_PREFIX_LENGTH] << 8) |
    bytes[offset + FRAME_PREFIX_LENGTH + 1]
...
function patchAppleWLoc(bytes, target, stats) {
  ...
  for (const field of fields) {
    if (field.fieldNo === 2 && field.wireType === 2) {
      ...
      (field.fieldNo === 22 || field.fieldNo === 24) &&
      field.wireType === 2
    ) {
      chunks.push(
        encodeField(
...
function patchLocation(bytes, target, stats) {
  ...
  const altitude =
    target.altitude == null ? null : Math.round(target.altitude * 1e4)
...
function collectAppleWLocLocations(bytes) {
  const fields = parseFields(bytes)
  const locations = []

  for (const field of fields) {
    if (field.fieldNo === 2 && field.wireType === 2) {
      collectWifiLocations(field.value, locations)
    } else if (
      (field.fieldNo === 22 || field.fieldNo === 24) &&
      field.wireType === 2
    ) {
      collectCellLocations(field.value, locations)
    }
  }
...
function collectWifiLocations(bytes, locations) {
  const fields = parseFields(bytes)
  const bssidField = fields.find(
    (field) =>
      field.fieldNo === 1 &&
      field.wireType === 2 &&
      MAC_RE.test(ascii(field.value)),
  )
  if (!bssidField) return

  for (const field of fields) {
    if (field.fieldNo === 2 && field.wireType === 2) {
      const location = decodeLocation(field.value)
      if (location)
        locations.push({
          source: `wifi:${ascii(bssidField.value)}`,
          ...location,
        })
    }
  }
...
function collectCellLocations(bytes, locations) {
  for (const field of parseFields(bytes)) {
    if (field.fieldNo === 5 && field.wireType === 2) {
      const location = decodeLocation(field.value)
      if (location) locations.push({ source: "cell", ...location })
    }
  }
}

function decodeLocation(bytes) {
  const location = {}

  for (const field of parseFields(bytes)) {
    if (field.wireType !== 0) continue
    const value = Number(BigInt.asIntN(64, field.value))

    if (field.fieldNo === 1) location.latitude = value / 1e8
    else if (field.fieldNo === 2) location.longitude = value / 1e8
    else if (field.fieldNo === 3) location.accuracy = value
    else if (field.fieldNo === 5) location.altitude = value / 1e4
  }
...
/**
 * https://chatgpt.com/share/6a4b266e-1854-83e8-b412-0d8a1f5a9620
 * @param {number} lat
 * @param {number} lng
 * @param {number} radius
 */
export function randomPoint(lat, lng, radius) {
